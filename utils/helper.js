...
exports.filterProducts = async (req, allProductsData,limit, skipItems) => {
  ...
  searchedAttributeIds = await Promise.all(searchedAttributeIds.map(async ({_id})=>_id.toString()));
  let searchedProducts = allProductsData.products.filter(product=>{
    let attribute = product.attributes.find(({value})=>searchedAttributeIds.includes(value.toString()));
