...
function DataTable() {
  ...
  const fetchData = async () => {
    var data = await axios.get < any > (`/api/admin/product/?page=${options?.page ?? 1}&limit=${options?.limit ?? 5}&sort=${options.sort ?? 'default'}`);
  ...
  var next = products.page == null ? 1 : products.page + 1;
  var prev = products.page == null ? 1 : products.page - 1;
  var pageSize = products.to;
  ...
  const deleteHandle = async (id = null) => {
    var deleteId = id != null ? id.split(",") : id;
    ...
    if (confirm(message)) {
      var data = await axios.delete < any > (`/api/admin/product?Id=${deleteId}`);
      if (data.status == 200) {
        fetchData();
        setSelectedItems(() => {
          return [];
  ...
  const editHandle = async () => {
    var updateId = productData.updateId != 'null' ? productData.updateId.split(",") : productData.updateId;
    if (updateId == '') {
      updateId = selectedItems;
    }
    var data = await axios.put < any > (`/api/admin/product?Id=${updateId}`, productData);
    if (data.status == 200) {
      fetchData();
      onClose();
