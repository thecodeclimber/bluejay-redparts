...
function Dropdown<T extends IDropdownItem>(props: Props<T>) {
  ...
  return (
    <div className={classes} ref={rootRef}>
      ...
      >
        {hasLabel && (
          <span style={{ color: '#9e9e9e' }} className="topbar__button-label">
            {label}
          </span>
        )}
        {hasTitle && (
        ...
        <span style={{ color: '#fff' }} className="topbar__button-arrow">
          <ArrowDownSm7x5Svg />
        </span>
      </button>
      <div className="topbar__menu-body">
        {items.map((item, index) => (
          ...
          >
            {!!item.image && <AppImage src={item.image} alt={item.title} />}
