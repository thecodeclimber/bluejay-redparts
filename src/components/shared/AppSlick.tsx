...
interface Props extends ISlickProps {
  ...
  forwardRef:
    | React.RefCallback<Slick | null>
    | React.MutableRefObject<Slick | null>
    | null;
}
...
function AppSlickInner(props: Props) {
  ...
  const [activeSlides, setActiveSlides] = useState(
    getActiveSlides(getStartPosition())
  );
  ...
  const beforeChangeWrapper: ISlickProps['beforeChange'] = useCallback(
    (oldIndex, newIndex) => {
      if (beforeChange) {
