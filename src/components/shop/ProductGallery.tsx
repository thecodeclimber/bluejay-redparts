...
function ProductGallery(props: Props) {
  ...
  const openPhotoswipe = (index: number) => {
    ...
    const items = imagesRefs.current.map((tag, index) => {
      ...
      const width =
        (tag.dataset.width ? parseFloat(tag.dataset.width) : null) ||
        tag.naturalWidth;
      const height =
        (tag.dataset.height ? parseFloat(tag.dataset.height) : null) ||
        tag.naturalHeight;
  ...
  const handleFeaturedBeforeChange: ISlickProps['beforeChange'] = (
    oldIndex,
    newIndex
  ) => {
    setState((prev) => ({
  ...
  // componentDidMount
  useEffect(() => {
    createGalleryRef.current = import('~/services/photoswipe').then(
  ...
  return (
    <ProductProductGallery data-layout={layout} {...rootProps}>
      <ProductGalleryFeatured>
        <ProductGalleryZoom type="button" onClick={handleZoomButtonClick}>
          <ZoomIn24Svg />
        </ProductGalleryZoom>
      ...
      <div className="product-gallery__thumbnails">
        <AppSlick {...slickSettingsThumbnails[layout]}>
          {images.map((image, index) => (
            <ProductGalleryThumbnailsItem
              active={index === state.currentIndex ? 1 : 0}
              type="button"
              key={index}
