...
import {
  ...
  ReviewsViewForm,
  ReviewsViewHeader,
} from '~/styled-components/shop/ReviewsView';
interface IForm {
  rating: string;
  author: string;
  email: string;
...
function ReviewsView(props: Props) {
  ...
  const load = async (
    productId: number,
    page: number,
    canceledRef?: React.RefObject<boolean>
  ) => {
    await shopApi
      .getProductReviews(productId, { limit: 8, page })
      .then((result) => {
  ...
  useEffect(() => {
    ...
    if (
      productId !== listMetaRef.current?.productId ||
  ...
  return (
    <ReviewsReviewsView>
      ...
      <ReviewsViewForm onSubmit={handleSubmit(submit)}>
        <ReviewsViewHeader>
        ...
        <div className="row">
          <div
            className={classNames({
              'col-xxl-8 col-xl-10 col-lg-9 col-12':
                productPageLayout === 'full',
