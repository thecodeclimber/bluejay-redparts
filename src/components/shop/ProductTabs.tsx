...
function ProductTabs(props: Props) {
  ...
  const tabs = useMemo<ITab[]>(
    () => [
      ...
      },
      {
        ...
        content: <Specification groups={spec} product={product} />,
      },
      {
        id: 'product-tab-reviews',
        title: intl.formatMessage({ id: 'TEXT_TAB_REVIEWS' }),
        content: (
          <ReviewsView productId={product.id} productPageLayout={layout} />
        ),
  ...
  const rootClasses = classNames(
    `product-tabs product-tabs--layout--${layout}`,
    className
  );
  ...
  return (
    <ProductProductTabs {...rootProps}>
      ...
      <ProductTabsList>
        {tabs.map((tab, index) => (
          <ProductTabsItem active={activeTab === tab.id} key={index}>
            <AppLink
              href={{ href: { hash: tab.id } }}
      ...
      <ProductTabsContent>
        {tabs.map((tab, index) => (
          <ProductTabsPane active={activeTab === tab.id} key={index}>
            {tab.content}
          </ProductTabsPane>
        ))}
      </ProductTabsContent>
