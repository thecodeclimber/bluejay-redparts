...
function getItemValues(items: VehicleSelectItem[]): any[] {
  return items.reduce<any[]>(
    (acc, prevItem) => [...acc, getItemValue(prevItem)],
    []
  );
}
...
function deserializeOption<T extends any>(
  option: string,
  item: VehicleSelectItem<T>
): T {
...
export default function useVehicleForm(options: IOptions = {}) {
    ...
    makeItems([
      {
        ...
        optionsSource: vehicleApi.getCategory.bind(vehicleApi),
      },
      {
        key: 'type',
        ...
        optionsSource: vehicleApi.getSubCategory.bind(vehicleApi),
      },
      {
        key: 'diameter',
        ...
        optionsSource: vehicleApi.getDiameter.bind(vehicleApi),
      },
      {
        key: 'length',
        ...
        optionsSource: vehicleApi.getLength.bind(vehicleApi),
      },
    ])
  );
  ...
  const onItemValueChange = (index: number, value: string): void => {
    ...
    if (value === 'none') {
      ...
      onChange(
        deserializeOption<IVehicle>(
          value,
          items[index] as VehicleSelectItem<IVehicle>
        )
      );
    }
  };
